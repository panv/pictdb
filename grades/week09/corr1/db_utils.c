            ...
            struct pictdb_file* db_file)
{
    ...
    if (read_els != 1) {
        ...
        //correcteur: pas de fclose ? voir mieux, do_close. Peut être que c'est fait a chaques fois qu'on appel do_open mais du coup c'est pas modularisé puisqu'on doit a chaques fois qu'on appel do_open vérifier et appeler do_close, juste vérifier serait mieux.
        return ERR_IO;
    }
    ...
    if (read_els != db_file->header.max_files) {
        ...
        //correcteur: ici aussi, avec un free.
