...
//correcteur: encore une fois ces maccros sont assez moyenne, pas résistante aux parametres malformés qu'on leur passe. Aussi elle diminuent la lisibilité.
...
int do_read_cmd(int args, char* argv[])
{
    ...
    if (ret == 0) {
        ...
        if (ret == 0) {
            ...
            //correcteur: ne pas abusé de se genre de notation, c'est pas tres lisible et ca ira pasplus vite a l'execution.
